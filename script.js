...
function drawPolarChart(options, appData) {
    ...
    var circleContainers = svg.append("g")
        .attr("class", "circle-containers"); 


    circleContainers.selectAll(".levels")
    //appending data to circleContainers
        .data(d3.range(1, options.levels + 1))
        .enter()
        .append("circle")
        .attr("class", "x-circles")
        .attr("r", function(d, i) { 
            // console.log(i)
            return (options.height/2) / options.levels * d; 
        })
